...
export default function AcceptInvitation() {
  ...
  const apiBase = "http://localhost:3001"; // Backend base URL

  // Remote data & flow state
  const [loading, setLoading] = useState(true);
  const [verifying, setVerifying] = useState(true);
  const [data, setData] = useState(null);
  const [error, setError] = useState(null); // { error: 'expired'|'revoked'|'already_accepted'|'invalid_token'|'unknown' }
  ...
  // Derivations for password rules & strength
  ...
  // Submit password to accept invitation
